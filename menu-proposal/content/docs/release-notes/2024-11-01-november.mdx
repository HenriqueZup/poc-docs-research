...
- [**Novo tipo de input `textarea`**](create-use/create-content/yaml-files/inputs#input-do-tipo-textarea). Este novo input é uma semelhante ao tipo **`text`** (texto), mas permite que você use textos extensos sem comprometer a visualização dos espaçamentos e quebras de linhas. Esta novidade traz uma melhora na experiência do preenchimento de inputs que recebem valores mais estruturados como trechos de código.
