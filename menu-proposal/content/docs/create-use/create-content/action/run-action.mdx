...
Para criar a estrutura inicial da sua Action, execute o comando [**`stk create action`**](home/stk-cli/commands/action-commands.mdx#stk-create-action) e responda no terminal as perguntas a seguir:
...
No **`action.yaml`** você deve usar a pasta [**component_path**](create-use/create-content/yaml-files/metadata.mdx#metadatacomponent_path-caminho-da-origem-de-plugin-é-o-local-onde-o-plugin-está-armazenado) para conseguir renderizar o conteúdo interpolado dos seus arquivos ou scripts da pasta **templates** junto aos conteúdos da Action.
...
Execute o comando [**stk run action**](home/stk-cli/commands/action-commands.mdx#stk-run-action).
...
**Passo 1.** Execute o comando [**stk validate action**](home/stk-cli/commands/action-commands.mdx#stk-validate-action) dentro da pasta da Action.
...
<CustomDocCardList>
  <CustomDocCard title="Publicar sua Action em um Estúdio" description="Confira como publicar a sua Action em um Estúdio." link="/create-use/create-content/action/publish-action.mdx" />
  <CustomDocCard title="Publicar uma Action com STK CLI" description="Confira os comandos para publicar uma action via STK CLI." link="/home/stk-cli/commands/action-commands.mdx" />
  <CustomDocCard title="Associe Actions à Plugins de Infraestrutura" description="Confira como associar Actions à Plugins de Infra." link="/create-use/create-content/action/action-infra-plugin.mdx" />
  <CustomDocCard title="Connection Interfaces" description="Confira mais detalhes sobre as Connections Interfaces." link="/create-use/connections/connection-interface.mdx" />
</CustomDocCardList>