...
O comando [**`stk create plugin`**](home/stk-cli/commands/plugin-commands.mdx#stk-create-plugin) simplifica a criação de Plugins de Infraestrutura a partir de códigos IaC (Infrastructure as Code) legados, como arquivos Terraform que você já possui.
...
Você deve tratar manualmente os valores que não foram extraídos em inputs para o seu Plugin. [**Para saber todas as opções do comando 'stk create plugin' consulte a página de comandos da StackSpot.**](home/stk-cli/commands/plugin-commands.mdx#stk-create-plugin)
