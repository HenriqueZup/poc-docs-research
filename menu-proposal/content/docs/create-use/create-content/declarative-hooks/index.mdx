...
As variáveis acessíveis são as definidas pelos [**inputs**](create-use/create-content/yaml-files/advanced-inputs.mdx#computed-inputss) **`computed-inputs`** do Plugin. No trecho de exemplo apresentado, é verificado se o valor do input **`language`** é igual à string **`java`**. Em caso positivo, são gerados os arquivos definidos pelos templates que estão na pasta **`templates-java`**. 
