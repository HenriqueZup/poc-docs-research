...
description: Informações sobre os tipos de Workflow. E as opções disponíveis para
  cada tipo.
---
...
Os outputs do Workflow possuem o comportamento semelhante ao **`computed-inputs`**. Para saber mais como usar os Outputs do Workflow, confira o conteúdo dos [**computed-inputs**](docs/create-use/create-content/yaml-files/advanced-inputs.mdx#computed-inputs).
...
spec:
  ...
  targets:
    - infra
  docs:
    en-us: docs/en_us/rollback.mdx
    pt-br: docs/pt_br/rollback.mdx
  inputs:
    - label: "Ambiente"
      scope: default
      name: environment_id
