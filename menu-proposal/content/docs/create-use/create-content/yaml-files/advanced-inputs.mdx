...
- Um problema pode ocorrer quando um [**Plugin de Infraestrutura com a opção `app-allowed: true`**](create-use/create-content/yaml-files/plugin-yaml.mdx#app-allowed) depende de um input avançado como os **inputs globais** ou **global-computed-inputs** de um Plugin de Aplicação.
...
Inputs globais são compatíveis no portal da StackSpot. Para mais informações, [**acesse o uso de Jinja no portal da StackSpot**](create-use/create-content/yaml-files/jinja.mdx#jinja-no-portal-da-stackspot).
...
Inputs **`global-computed-inputs`** aceita **expressões Jinja** e são compatíveis no portal da StackSpot. Para mais informações, [**acesse o uso de Jinja no portal da StackSpot**](create-use/create-content/yaml-files/jinja.mdx#jinja-no-portal-da-stackspot).
...
Por exemplo, para usar este valor em um arquivo **README.mdx** que seria criado pelo Plugin:
